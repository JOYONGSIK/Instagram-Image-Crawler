...
from detection.config import MTCNNConfig
...
class MTCNN:
    """
    Allows to perform Detection ->
        a) Detection of faces (with the confidence probability)
        b) Detection of keypoints (left eye, right eye, nose, mouth_left,
            mouth_right)
    """
    def __init__(self, min_face_size: int = 20, scale_factor: float = 0.709):
        """
        F
        :param weights_file: file uri with the weights of the P, R and O \
            networks. By default it will load
        ...
        self._steps_threshold = config.STEPS_THRESHOLD
        self._scale_factor = scale_factor

        self._pnet, self._rnet, self._onet = \
            NetworkFactory().build_P_R_O_nets_from_file(config.MODEL_PATH)
    ...
    @staticmethod
    def __scale_image(img, scale: float):
        """
        Scales the img to a given scale.
        :param img:
        :param scale:
        :return:
        """
        ...
        im_data, _ = Image.resize(img=img,
                                  target_size=(width_scaled, height_scaled))
    ...
    def __generate_bounding_box(imap, reg, scale, t):
        ...
        reg = np.transpose(np.vstack(
            [dx1[(y, x)], dy1[(y, x)], dx2[(y, x)], dy2[(y, x)]]))
        if reg.size == 0:
    ...
    def detect(self, img, threshold=0, **kwargs) -> list:
        """
        Detects bounding boxes from the specified image.
        :param img: image to process
        :return: list containing all the bounding boxes detected with their \
            keypoints.
        """
        Image.is_valid(img)
        img = Image.convert_BGR2RGB(img.copy())
        height, width, _ = img.shape
    ...
    def __stage2(self, img, total_boxes, stage_status: StageStatus):
        ...
        for k in range(0, num_boxes):
            ...
            dy = stage_status.dy[k]
            edy = stage_status.edy[k]
            dx = stage_status.dx[k]
            edx = stage_status.edx[k]
            y = stage_status.y[k]
            ey = stage_status.ey[k]
            x = stage_status.x[k]
            ex = stage_status.ex[k]
            tmp[dy-1:edy, dx-1:edx, :] = img[y-1:ey, x-1:ex, :]
            if ((tmp.shape[0] > 0) and
                    (tmp.shape[1] > 0) or
                    (tmp.shape[0] == 0) and
                    (tmp.shape[1] == 0)):
                tempimg[:, :, :, k], _ = Image.resize(tmp, (24, 24))
            else:
    ...
    def __stage3(self, img, total_boxes, stage_status: StageStatus):
        ...
        for k in range(0, num_boxes):
            ...
            if ((tmp.shape[0] > 0) and
                    ...
                    (tmp.shape[0] == 0) and
                    (tmp.shape[1] == 0)):
                tempimg[:, :, :, k], _ = Image.resize(tmp, (48, 48))
            else:
...
class NetworkFactory:
    def build_pnet(self, input_shape=None):
        ...
        p_layer = MaxPooling2D(pool_size=(2, 2), strides=(2, 2),
    ...
    def build_rnet(self, input_shape=None):
        ...
        r_layer = Conv2D(28, kernel_size=(3, 3), strides=(1, 1),
                         padding="valid")(r_inp)
        r_layer = PReLU(shared_axes=[1, 2])(r_layer)
        r_layer = MaxPooling2D(pool_size=(3, 3), strides=(2, 2),
                               padding="same")(r_layer)

        r_layer = Conv2D(48, kernel_size=(3, 3), strides=(1, 1),
                         padding="valid")(r_layer)
        r_layer = PReLU(shared_axes=[1, 2])(r_layer)
        r_layer = MaxPooling2D(pool_size=(3, 3), strides=(2, 2),
                               padding="valid")(r_layer)

        r_layer = Conv2D(64, kernel_size=(2, 2), strides=(1, 1),
                         padding="valid")(r_layer)
        r_layer = PReLU(shared_axes=[1, 2])(r_layer)
    ...
    def build_onet(self, input_shape=None):
        ...
        o_layer = Conv2D(32, kernel_size=(3, 3), strides=(1, 1),
                         padding="valid")(o_inp)
        o_layer = PReLU(shared_axes=[1, 2])(o_layer)
        o_layer = MaxPooling2D(pool_size=(3, 3), strides=(2, 2),
                               padding="same")(o_layer)

        o_layer = Conv2D(64, kernel_size=(3, 3), strides=(1, 1),
        ...
        o_layer = Conv2D(64, kernel_size=(3, 3), strides=(1, 1),
                         padding="valid")(o_layer)
        o_layer = PReLU(shared_axes=[1, 2])(o_layer)
        o_layer = MaxPooling2D(pool_size=(2, 2), strides=(2, 2),
                               padding="same")(o_layer)

        o_layer = Conv2D(128, kernel_size=(2, 2), strides=(1, 1),
                         padding="valid")(o_layer)
        o_layer = PReLU(shared_axes=[1, 2])(o_layer)
